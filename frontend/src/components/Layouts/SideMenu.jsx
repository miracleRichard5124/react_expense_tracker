...
const SideMenu = ({activeMenu}) => {
  ...
  return <div className='w-64 h-[calc(100vh-61px)] bg-white border-r border-gray-200/50 p-5 sticky top-[61px] z-20'>
      <div className="flex flex-col items-center justify-center gap-3 mt-3 mb-7">
        {user?.profileImageUrl ? (
          <img
            ...
            alt='Profile Image'
            className='w-36 rounded-full bg-slate-400'
          />
      ...
      {SIDE_MENU_DATA.map((item, index) => (
        <button
          key={`menu_${index}`}
          className={`w-full flex items-center gap-4 text-[15px] cursor-pointer ${activeMenu == item.label ? "text-white bg-primary" : ""} px-6 py-3 rounded-lg mb-3`}
